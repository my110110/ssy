...
<?php if(empty($model)):?>
    ...
    <?php else:?>
                        ...
                        <tr class="shows success">
                            ...
                            <td>
                                <?= Html::a('<button type="button" class="btn btn-default btn-xs">查看</button>', ['view', 'id' => $model['id'],'type'=>$type], ['title'=>'查看']) ?>
                                <?= Html::a('<button type="button" class="btn btn-primary btn-xs">下载</button>', ['export','id' => $model['id'],'type'=>$type], ['title'=>'下载']) ?>
                                        <?php if(AdminUser::getUserRole(yii::$app->user->id)==1):?>
                                <?= Html::a('<button type="button" class="btn btn-warning btn-xs">修改</button>', ['update', 'id' => $model['id'],'type'=>$type], ['title'=>'修改']) ?>
                                 <?= Html::a('<button type="button" class="btn btn-danger btn-xs">删除</button>', ['del', 'id' => $model['id'],'type'=>$type], [
                                    'title'=>'删除',
                                    'data' => [
                                        'confirm' => '确定要删除这个项目吗?',
                                        'method' => 'post',
